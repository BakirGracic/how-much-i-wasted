...
export default function Picker() {
	...
	const validate = () => {
		...
		if (!formData.year.match(/^\d{4}$/) || parseInt(formData.year) < 1000 || parseInt(formData.year) > currentYear) {
	...
	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		if (validate()) {
			const dateTime = new Date(parseInt(formData.year), parseInt(formData.month) - 1, parseInt(formData.day), parseInt(formData.hour) || 0, parseInt(formData.minute) || 0);
			localStorage.setItem(process.env.NEXT_PUBLIC_LS_KEY || '', dateTime.toISOString());
			setDtValid(true);
	...
	return (
		...
		>
			...
			<div className='mx-auto max-w-[300px] text-left'>
				...
				{errors.minute && <p className='text-red-500'>{errors.minute}</p>}
			</div>
